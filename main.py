...
MAIN_MARKUP = ReplyKeyboardMarkup(
    ...
    input_field_placeholder="Send link, video or audio..."
)
...
async def send_welcome(client, message):
    if await ensure_joined(client, message):
        ...
        await message.reply_text(welcome_text, quote=True, reply_markup=MAIN_MARKUP)
...
async def set_key_plain(client, message):
    ...
    await message.reply_text(msg, quote=True, reply_markup=MAIN_MARKUP)
    if not prev:
        try:
...
async def handle_media(client, message):
    ...
    if not user_key:
        await message.reply_text("first send me Gemini key 🤓", quote=True, reply_markup=ReplyKeyboardMarkup([["/help"]], resize_keyboard=True, input_field_placeholder="Paste your Gemini Key here..."))
        return
    ...
    try:
        ...
        await message.reply_text(f"❌ Error: {e}", quote=True, reply_markup=MAIN_MARKUP)
    ...
    if len(text) > MAX_MESSAGE_CHUNK:
        if mode == "Split messages":
            ...
            for i in range(0, len(text), MAX_MESSAGE_CHUNK):
                sent = await client.send_message(chat_id, text[i:i+MAX_MESSAGE_CHUNK], reply_to_message_id=reply_id, reply_markup=MAIN_MARKUP)
            return sent
        else:
            fname = os.path.join(DOWNLOADS_DIR, f"{action}.txt")
            with open(fname, "w", encoding="utf-8") as f: f.write(text)
            sent = await client.send_document(chat_id, fname, caption="Open this file and copy the text inside 👍", reply_to_message_id=reply_id, reply_markup=MAIN_MARKUP)
            try: os.remove(fname)
            except: pass
            return sent
    return await client.send_message(chat_id, text, reply_to_message_id=reply_id, reply_markup=MAIN_MARKUP)
