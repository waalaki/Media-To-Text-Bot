...
MAX_UPLOAD_MB = int(os.environ.get("MAX_UPLOAD_MB", "90"))
...
def get_audio_duration_sync(input_path):
    try:
...
def split_audio_sync(input_path):
    try:
        duration = get_audio_duration_sync(input_path)
        if duration <= 1800:
            return [input_path]
        base = os.path.splitext(input_path)[0]
        dirname = os.path.dirname(input_path)
        basename_base = os.path.basename(base)
        out_pattern = os.path.join(dirname, f"{basename_base}_part_%03d.mp3")
        cmd = ["ffmpeg", "-i", input_path, "-f", "segment", "-segment_time", "1800", "-ac", "1", "-ar", "16000", "-b:a", "32k", "-y", out_pattern]
        subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        parts = []
        for f in sorted(os.listdir(dirname)):
            if f.startswith(f"{basename_base}_part_") and f.endswith(".mp3"):
                parts.append(os.path.join(dirname, f))
        if parts:
            try:
...
async def handle_media(client, message):
    ...
    try:
        ...
        async with SEMAPHORE:
            ...
            loop = asyncio.get_running_loop()
            parts = await loop.run_in_executor(EXECUTOR, split_audio_sync, final_path)
            texts = []
            for part in parts:
                try:
                    t = await loop.run_in_executor(EXECUTOR, upload_and_transcribe_gemini, part, user_key, message.from_user.id)
                    if t:
                        texts.append(t)
                except Exception as e:
                    logging.exception("Transcription part error: %s", e)
