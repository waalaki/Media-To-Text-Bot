...
async def progress_updater(client, chat_id, msg_id, event):
    pct = 0
    start = time.time()
    while not event.is_set():
        ...
        if pct < 70:
            ...
            pct = min(95, pct + 1)
        try:
            await client.edit_message_text(chat_id, msg_id, f"Processing... {pct}%")
        except:
            pass
        await asyncio.sleep(1)
    try:
        await client.edit_message_text(chat_id, msg_id, "Processing... 100%")
...
async def handle_media(client, message):
    ...
    try:
        ...
        progress_msg = await client.send_message(message.chat.id, "Processing... 0%")
        progress_task = asyncio.create_task(progress_updater(client, message.chat.id, progress_msg.id, progress_event))
        async with SEMAPHORE:
            ...
            try:
                ...
                await message.reply_text("❌ Transcription timed out.", quote=True)
                return
        if text:
        ...
        try:
            progress_event.set()
            if progress_task:
                await asyncio.wait_for(progress_task, 5)
        except:
            pass
        try:
            if progress_msg:
                ...
                try:
                    await client.delete_messages(message.chat.id, progress_msg.id)
                except:
                    pass
        except:
            pass
...
async def process_text_action(client, call, origin_msg_id, log_action, prompt_instr):
    ...
    progress_task = None
    try:
        progress_msg = await client.send_message(chat_id, "Processing... 0%")
        progress_task = asyncio.create_task(progress_updater(client, chat_id, progress_msg.id, progress_event))
        async with SEMAPHORE:
            loop = asyncio.get_running_loop()
            ...
            try:
                ...
                await client.send_message(chat_id, "❌ Operation timed out.")
                return
