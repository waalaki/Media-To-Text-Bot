...
async def delete_account(client, message):
    ...
    try:
        ...
        logging.warning("Failed to delete user from DB: %s", e)
    user_gemini_keys.pop(uid, None)
    user_mode.pop(uid, None)
    action_usage.pop(uid, None)
    user_model_usage.pop(uid, None)
    for chat_id in list(user_transcriptions.keys()):
        try:
            user_transcriptions.pop(chat_id, None)
        except:
            pass
    await message.reply_text("Your data has been removed. You are now a new user.", quote=True)
