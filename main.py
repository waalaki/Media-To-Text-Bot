...
try:
    ...
    if db is not None:
        ...
        cursor = users_col.find({}, {"user_id": 1, "gemini_key": 1, "custom_prompt": 1})
        for doc in cursor:
            user_gemini_keys[int(doc["user_id"])] = doc.get("gemini_key")
            if doc.get("custom_prompt"):
                user_custom_prompts[int(doc["user_id"])] = doc.get("custom_prompt")
...
def set_user_custom_prompt_db(uid, prompt):
    try:
        if users_col is not None:
            users_col.update_one({"user_id": uid}, {"$set": {"custom_prompt": prompt, "updated_at": time.time()}}, upsert=True)
        if prompt:
            user_custom_prompts[uid] = prompt
        else:
            user_custom_prompts.pop(uid, None)
    except Exception as e:
        logging.warning("Failed to set custom prompt in DB: %s", e)
        if prompt:
            user_custom_prompts[uid] = prompt
        else:
            user_custom_prompts.pop(uid, None)
...
def upload_and_transcribe_gemini(file_path: str, key: str, uid: int) -> str:
    ...
    try:
        ...
        prompt = user_custom_prompts.get(uid, "Transcribe the audio in this file Provide a clean text that does not look like raw STT. Return ONLY the transcription text, no preamble or extra commentary")
...
async def send_welcome(client, message):
    if await ensure_joined(client, message):
        ...
        main_kb = ReplyKeyboardMarkup(
            [
                ["Change Result mode 🐥", "Custom Prompt ⚙️"]
            ],
...
@app.on_message(filters.regex(r"^Custom Prompt ⚙️$"))
async def custom_prompt_menu(client, message):
    if not await ensure_joined(client, message):
        return
    uid = message.from_user.id
    current = user_custom_prompts.get(uid)
    if current:
        kb = ReplyKeyboardMarkup([["Clear Custom Prompt", "Cancel"]], resize_keyboard=True, placeholder="Custom prompt active")
        await message.reply_text("You have a custom prompt set. Send new prompt to replace it, or press Clear Custom Prompt.", reply_markup=kb, quote=True)
        user_state[uid] = "awaiting_custom_prompt"
    else:
        kb = ReplyKeyboardMarkup([["Cancel"]], resize_keyboard=True, placeholder="Set custom prompt")
        await message.reply_text("Send the custom prompt you want to use for transcriptions. To cancel press Cancel.", reply_markup=kb, quote=True)
        user_state[uid] = "awaiting_custom_prompt"

@app.on_message(filters.regex(r"^Clear Custom Prompt$"))
async def clear_custom_prompt_btn(client, message):
    uid = message.from_user.id
    set_user_custom_prompt_db(uid, None)
    user_state.pop(uid, None)
    main_kb = ReplyKeyboardMarkup([["Change Result mode 🐥", "Custom Prompt ⚙️"]], resize_keyboard=True, placeholder="Message")
    await message.reply_text("Custom prompt cleared. Default prompt will be used.", reply_markup=main_kb, quote=True)

@app.on_message(filters.regex(r"^Cancel$"))
async def cancel_custom_prompt_btn(client, message):
    uid = message.from_user.id
    user_state.pop(uid, None)
    main_kb = ReplyKeyboardMarkup([["Change Result mode 🐥", "Custom Prompt ⚙️"]], resize_keyboard=True, placeholder="Message")
    await message.reply_text("Cancelled.", reply_markup=main_kb, quote=True)
...
async def handle_text_inputs(client, message):
    ...
    if state == "awaiting_custom_prompt":
        ...
        if text.lower() == "cancel":
            ...
            await message.reply_text("Cancelled.", reply_markup=main_kb, quote=True)
            return
        set_user_custom_prompt_db(uid, text)
        user_state.pop(uid, None)
        main_kb = ReplyKeyboardMarkup([["Change Result mode 🐥", "Custom Prompt ⚙️"]], resize_keyboard=True, placeholder="Message")
        await message.reply_text("Custom prompt saved. It will be used for future transcriptions. To clear use the Custom Prompt button and press Clear Custom Prompt.", reply_markup=main_kb, quote=True)
        return
    if re.match(r"^/clearprompt$", message.text or "", re.IGNORECASE):
        set_user_custom_prompt_db(uid, None)
        await message.reply_text("Custom prompt cleared.", quote=True)
        return
...
async def handle_media(client, message):
    ...
    if not media or getattr(media, "file_size", 0) > MAX_UPLOAD_SIZE:
        if media:
            await message.reply_text(f"Send me file less than {MAX_UPLOAD_MB}MB 😎", quote=True)
        return
