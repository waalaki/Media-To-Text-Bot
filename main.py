...
def split_audio_to_chunks(input_path, chunk_seconds=1800):
    base = os.path.splitext(input_path)[0]
    pattern = f"{base}_part%03d.mp3"
    cmd = ["ffmpeg", "-i", input_path, "-f", "segment", "-segment_time", str(chunk_seconds), "-c", "copy", "-reset_timestamps", "1", "-y", pattern]
    try:
        subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        parts = sorted(glob.glob(f"{base}_part*.mp3"))
        return parts
    except Exception:
        return []
...
def upload_and_transcribe_with_chunks(file_path: str, key: str, uid: int, chunk_seconds: int = 1800) -> str:
    ...
    parts = split_audio_to_chunks(file_path, chunk_seconds=chunk_seconds)
    if not parts:
        return upload_and_transcribe_gemini(file_path, key, uid)
    texts = []
    for part in parts:
        try:
            txt = upload_and_transcribe_gemini(part, key, uid)
            if txt:
                texts.append(txt)
        except Exception:
            pass
...
async def _progress_updater(client, chat_id, reply_to, future, label="Processing"):
    percent = 0
    bars = 12
    bar_empty = "░"
    bar_full = "█"
    try:
        progress_msg = await client.send_message(chat_id, f"{label}: 0% [{bar_empty * bars}]", reply_to_message_id=reply_to)
    except Exception:
        progress_msg = None
    try:
        ...
        if progress_msg:
            try:
                await client.edit_message_text(chat_id, progress_msg.id, final_text)
            except Exception:
                pass
            try:
...
async def handle_media(client, message):
    ...
    try:
        ...
        async with SEMAPHORE:
            ...
            loop = asyncio.get_running_loop()
            text_task = loop.run_in_executor(EXECUTOR, upload_and_transcribe_with_chunks, final_path, user_key, message.from_user.id, 1800)
            progress_task = asyncio.create_task(_progress_updater(client, message.chat.id, message.id, text_task, label="Transcribing"))
...
async def process_text_action(client, call, origin_msg_id, log_action, prompt_instr):
    ...
    try:
        async with SEMAPHORE:
            ...
            progress_task = asyncio.create_task(_progress_updater(client, chat_id, data.get("origin", call.message.id), ask_task, label=log_action))
