...
def _detect_mime_type(path):
    ...
    if ext in ['.m4a', '.mp4', '.mp3', '.wav', '.ogg', '.webm', '.aac', '.flac']:
        mapping = {
            '.m4a': 'audio/mp4',
            '.mp4': 'video/mp4',
            '.mp3': 'audio/mpeg',
...
def upload_and_transcribe_gemini(file_path: str, key: str, uid: int) -> str:
    ...
    try:
        ...
        with open(file_path, "rb") as fh:
            try:
                uploaded_file = client.files.upload(file=fh, mime_type=mime_type)
            except TypeError:
                uploaded_file = client.files.upload(file=fh)
...
def handle_media(message):
    ...
    try:
        ...
        local_path = os.path.join(DOWNLOADS_DIR, f"temp_{message.message_id}_{media.file_unique_id}{os.path.splitext(file_path)[1]}")
