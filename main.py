...
def upload_and_process_gemini(file_path: str, key: str, uid: int, is_image: bool = False) -> str:
    client = genai.Client(api_key=key)
    uploaded_file = None
    try:
        uploaded_file = client.files.upload(file=file_path)
        if is_image:
            prompt = "Extract all text from this image accurately. If there is no text, describe the image briefly."
        else:
...
def build_action_keyboard(text_len):
    if text_len > 1000:
        return InlineKeyboardMarkup([[InlineKeyboardButton("Get Summarize", callback_data="summarize_menu|")]])
    return None
...
async def set_key_plain(client, message):
    ...
    msg = "API key updated." if prev else "Okay send me audio, video or photo 👍"
...
async def handle_media(client, message):
    ...
    f_id = getattr(media, "file_unique_id", str(time.time()))
    file_path = os.path.join(DOWNLOADS_DIR, f"temp_{message.id}_{f_id}")
    ...
    try:
        ...
        text = await asyncio.get_event_loop().run_in_executor(None, upload_and_process_gemini, final_path, user_key, message.from_user.id, is_image)
        
        if text:
            kb = build_action_keyboard(len(text))
            sent = await client.send_message(message.chat.id, text, reply_to_message_id=message.id, reply_markup=kb)
            if sent:
                user_transcriptions.setdefault(message.chat.id, {})[sent.id] = {"text": text, "origin": message.id}
    except Exception as e:
        await message.reply_text(f"❌ Error: {e}", quote=True)

async def send_long_text(client, chat_id, text, reply_id, uid, action="Result"):
