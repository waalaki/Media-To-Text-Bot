...
REQUEST_TIMEOUT_GEMINI = int(os.environ.get("REQUEST_TIMEOUT_GEMINI", "120"))
...
async def handle_media(client, message):
    ...
    status_msg = await message.reply_text("⏳ Processing starting...", quote=True)
    file_path = os.path.join(DOWNLOADS_DIR, f"temp_{message.id}_{media.file_unique_id}")
    
    try:
        start_time = time.time()
        d_path = await client.download_media(
            message, 
            file_name=file_path,
        ...
        await status_msg.edit_text("⚙️ Optimizing audio...")
        async with SEMAPHORE:
            ...
            await status_msg.edit_text("🤖 AI is transcribing...")
            loop = asyncio.get_running_loop()
            ...
            try:
                ...
                await status_msg.edit_text("❌ Transcription timed out.")
                return
...
async def process_text_action(client, call, origin_msg_id, log_action, prompt_instr):
    ...
    status_msg = await client.send_message(chat_id, "⏳ AI is thinking...", reply_to_message_id=call.message.id)
    try:
        async with SEMAPHORE:
            loop = asyncio.get_running_loop()
            ...
            try:
                ...
                await status_msg.edit_text("❌ Operation timed out.")
                return
        await status_msg.delete()
