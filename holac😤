...
def _split_audio_sync(input_path, segment_seconds=2400):
    base = os.path.splitext(input_path)[0]
    out_pattern = f"{base}_part%03d.mp3"
    cmd = [
        "ffmpeg",
        "-i", input_path,
        "-vn",
...
def transcribe_with_chunking(file_path: str, key: str, uid: int, segment_seconds: int = 2400) -> str:
    try:
        duration = _get_duration_sync(file_path)
        if duration <= 0 or duration <= segment_seconds:
            return upload_and_transcribe_gemini(file_path, key, uid)
        parts = _split_audio_sync(file_path, segment_seconds)
        if not parts:
            return upload_and_transcribe_gemini(file_path, key, uid)
        texts = []
        for part in parts:
...
async def handle_media(client, message):
    ...
    try:
        ...
        async with SEMAPHORE:
            ...
            loop = asyncio.get_running_loop()
            text_task = loop.run_in_executor(EXECUTOR, transcribe_with_chunking, final_path, user_key, message.from_user.id)
